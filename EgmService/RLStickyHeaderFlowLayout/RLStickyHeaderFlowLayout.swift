...
extension RLStickyHeaderFlowLayout {
    ...
    override open func layoutAttributesForSupplementaryView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if (attributes == nil) && (elementKind == RLStickyHeaderParallaxHeader) {
            attributes = RLStickyHeaderFlowLayoutAttributes(forSupplementaryViewOfKind:elementKind, with:indexPath)
        }
        return attributes
    }
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        if collectionView!.dataSource != nil {
            ...
            if visibleParallexHeader && CGSize.zero.equalTo(parallaxHeaderReferenceSize) == false {
                let currentAttribute = RLStickyHeaderFlowLayoutAttributes(forSupplementaryViewOfKind:RLStickyHeaderParallaxHeader, with:IndexPath.init(index: 0))
